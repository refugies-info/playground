...
import type { LheoDocument, Action, Formation, Session } from "../../../packages/rco/src/lheo-types";
...
async function main() {
  ...
  if (!xmlPath) {
    console.error("Usage: pnpm tsx .skills/metadata/scripts/map-metadata.ts <path_to_xml>");
    process.exit(1);
  }
...
function mapLheoToRi(doc: LheoDocument) {
  ...
  const abstract = rawAbstract.length > 50 ? rawAbstract.substring(0, 47) + "..." : rawAbstract;

  // 3. Location
  const location = getText(action["lieu-de-formation"]?.[0]?.coordonnees?.adresse?.departement) || "";

  // 4. Price Logic
  const conventionnement = getText(action.conventionnement);
  const codeFinanceur = getText(action["organisme-financeur"]?.[0]?.["code-financeur"]);
  const isGratuit = conventionnement === "1" && FINANCEURS_PUBLICS.includes(codeFinanceur || "");
  const price = {
  ...
  const map = lieu ? {
    ...
    lat: parseFloat(getText(lieu.coordonnees.adresse.geolocalisation?.latitude) || "0"),
    lng: parseFloat(getText(lieu.coordonnees.adresse.geolocalisation?.longitude) || "0")
  } : null;
