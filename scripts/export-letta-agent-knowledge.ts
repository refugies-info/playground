...
const DEFAULT_EXCLUDED_SOURCE_NAMES = new Set([
  "ressources_exemples_redaction",
]);
const DEFAULT_EXCLUSION_REASON =
  "source retirée du corpus cible après revue qualité";
...
async function main(): Promise<void> {
  ...
  const agentId = options.fromFile
    ? "local-export-file"
    : readPlaygroundEnv("PLAYGROUND_AGENT_ID");
  ...
  const { excludedResources, resources } = excludeResources(
    normalizeResources(agentExport),
  );
...
function readPlaygroundEnv(
  name: "PLAYGROUND_AGENT_ID" | "PLAYGROUND_LETTA_API_KEY",
): string {
  const value = process.env[name];
  if (!value) {
    const forbiddenFallback =
      name === "PLAYGROUND_LETTA_API_KEY"
        ? "LETTA_API_KEY"
        : "LETTA_PROJECT_ID";
    throw new Error(
...
async function readExportFile(filePath: string): Promise<JsonRecord> {
  ...
  if (!isRecord(parsed)) {
    throw new Error(
      `Le fichier d'export ${filePath} ne contient pas d'objet JSON`,
    );
  }
  return parsed;
}
...
function normalizeResources(agentExport: JsonRecord): ExportedResource[] {
  ...
  const sourcesById = new Map(
    sources
      .filter(isRecord)
      .map((source) => [readOptionalString(source.id), source] as const),
  );
  ...
  return files.filter(isRecord).map((file, index) => {
    const sourceId =
      readOptionalString(file.source_id) || readOptionalString(file.folder_id);
    const source = sourceId ? sourcesById.get(sourceId) : undefined;
    const sourceName = source
      ? readOptionalString(source.name) ||
        readOptionalString(source.folder_name)
      : undefined;
    const fileName =
...
function excludeResources(resources: ExportedResource[]): {
  ...
  for (const resource of resources) {
    if (
      resource.sourceName &&
      DEFAULT_EXCLUDED_SOURCE_NAMES.has(resource.sourceName)
    ) {
...
function toTargetPath(
  ...
  const mappedSourceFolder =
    SOURCE_FOLDER_MAP[normalizedSourceFolder] ||
    slugifyPathSegment(sourceFolder);
  const rawRelativeSegments =
    segments.length > 1 ? segments.slice(1) : [makeSafePathSegment(fileName)];
  const relativeSegments =
    rawRelativeSegments[0] === sourceFolder
      ? rawRelativeSegments.slice(1)
      : rawRelativeSegments;
...
function makeSafeRelativePath(value: string): string {
  const segments = normalizeSlashes(value)
    .split("/")
    .filter(Boolean)
    .map(makeSafePathSegment);
...
function getWeakExtractionReasons(input: {
  ...
  if (
    ...
    reasons.push(
      `chunks indexés incomplets (${input.chunksEmbedded}/${input.totalChunks})`,
    );
  }

  if (
    isPdf(input.fileType, input.originalExtension) &&
    (input.totalChunks || 0) <= 1
  ) {
...
async function writeResources(input: {
  ...
  for (const resource of input.resources) {
    ...
    await writeFile(
      targetPath,
      renderResource(resource, input.exportedAt),
      "utf8",
    );
...
function renderResource(
  ...
  if (extension === ".md") {
    return renderMarkdownResource(
      { ...resource, content: redactedContent },
      exportedAt,
    );
  }
...
function redactContent(content: string): string {
  return CONTENT_REDACTIONS.reduce(
    (redactedContent, [pattern, replacement]) =>
      redactedContent.replace(pattern, replacement),
    content,
  );
}
...
function parseJsonArrayStart(
  line: string,
): { indent: string; propertyPrefix: string } | null {
...
function isPrimitiveJsonValue(value: string): boolean {
  try {
    ...
    return (
      parsedValue === null ||
      ["boolean", "number", "string"].includes(typeof parsedValue)
    );
  } catch {
    return false;
  }
...
function renderMarkdownResource(
  resource: ExportedResource,
  exportedAt: string,
): string {
  const title = path.basename(
    resource.fileName,
    path.extname(resource.fileName),
  );
  const audience = inferAudience(resource.targetPath);
  const description = inferDescription(resource);
  const frontmatter = {
    audience,
...
/**
 * Derive a `description` (required by the playground's `validate:corpus`
 * script — `pnpm validate:corpus` — for qmd indexing) from the Letta
 * Cloud export metadata. Keeps it human-readable and short enough for
 * a single-line frontmatter entry.
 */
function inferDescription(resource: ExportedResource): string {
  const sourceName = resource.sourceName ?? "Letta Cloud";
  const fileType = (resource.fileType ?? "").toLowerCase();
  if (
    fileType === "application/pdf" ||
    resource.fileName?.toLowerCase().endsWith(".pdf")
  ) {
    return `Document PDF exporté depuis Letta Cloud (source : ${sourceName}) et converti en Markdown.`;
  }
  if (
    fileType === "application/json" ||
    resource.fileName?.toLowerCase().endsWith(".json")
  ) {
    return `Schéma JSON exporté depuis Letta Cloud (source : ${sourceName}) — référence structurelle.`;
  }
  if (
    fileType === "text/csv" ||
    resource.fileName?.toLowerCase().endsWith(".csv")
  ) {
    return `Tableur CSV exporté depuis Letta Cloud (source : ${sourceName}) — données de référence.`;
  }
  return `Document exporté depuis Letta Cloud (source : ${sourceName}).`;
}

/**
 * Derive an `audience` value (required by `validate:corpus`) from the
 * target path. The playground's corpus puts agent-facing instructions
 * (skills, prompts) and human-facing references side-by-side, so we
 * default to `agent` for everything exported from Letta Cloud — the
 * redaction and redaction-validation passes are authored for agent
 * consumption.
 */
function inferAudience(_targetPath: string): "agent" | "human" | "both" {
  // All Letta Cloud exports are agent-facing by default. Future PRs
  // may detect human-facing content (e.g. user guides) by path pattern.
  return "agent";
}
...
function buildManifest(input: {
  ...
  const chunksEmbedded = sum(
    input.resources.map((resource) => resource.chunksEmbedded),
  );
  const totalChunks = sum(
    input.resources.map((resource) => resource.totalChunks),
  );
  const weakExtractionWarnings = input.resources.filter(
    (resource) => resource.weakExtractionReasons.length > 0,
  ).length;
  const missingContent = input.resources.filter(
    (resource) => !resource.content?.trim(),
  ).length;
...
function printSummary(manifest: ExportManifest, dryRun: boolean): void {
  ...
  console.log(
    `Fichiers sans contenu extrait : ${manifest.summary.missingContent}`,
  );
  console.log(
    `Alertes d'extraction faible : ${manifest.summary.weakExtractionWarnings}`,
  );
