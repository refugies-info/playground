-- Migration: Move assignee_id from editorial_records to workflows
-- Rationale (RI-1340): assignment must be possible from ingestion onward, before an
-- editorial_record exists. The workflow is the document spine — created at ingestion,
-- 1:1 with the document, and Realtime-enabled — so it is the correct home for assignee.
-- This unblocks assigning documents still in "à traiter" with no editorial_record.
-- Note: translation ownership stays on translation_records.author_id (separate concern).

-- 1. Add assignee_id to workflows
ALTER TABLE public.workflows
  ADD COLUMN assignee_id uuid;

-- 2. Foreign key → profiles (mirror previous editorial_records behavior)
ALTER TABLE public.workflows
  ADD CONSTRAINT workflows_assignee_id_fkey
  FOREIGN KEY (assignee_id)
  REFERENCES public.profiles(id)
  ON UPDATE CASCADE
  ON DELETE SET NULL;

-- 3. Index for sort/filter
CREATE INDEX IF NOT EXISTS workflows_assignee_id_idx
  ON public.workflows (assignee_id);

-- 4. Backfill from existing editorial_records assignments
UPDATE public.workflows w
SET assignee_id = er.assignee_id
FROM public.editorial_records er
WHERE er.id = w.editorial_record_id
  AND er.assignee_id IS NOT NULL;

-- 5. Recreate workflows_enriched view — assignee now sourced from workflows.assignee_id.
--    Must run BEFORE dropping editorial_records.assignee_id (view depended on it).
...
COMMENT ON VIEW workflows_enriched IS
    'Enriched view of workflows combining computed statuses, presentation metadata, and publication info.
     ...
     RI-1340: assignee moved from editorial_records to workflows (workflow_assignee_id); assignee_email/assignee_profile now follow workflows.assignee_id.';

-- 6. Drop the old assignee column on editorial_records (no longer referenced by the view).
ALTER TABLE public.editorial_records
  DROP CONSTRAINT IF EXISTS editorial_records_assignee_id_fkey;

DROP INDEX IF EXISTS public.editorial_records_assignee_id_idx;
