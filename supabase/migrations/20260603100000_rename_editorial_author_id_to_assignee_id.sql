-- Migration: Rename editorial_records.author_id to assignee_id
-- Rationale: the column tracks who is assigned to the editorial workflow,
-- not strictly the original author; renaming avoids semantic confusion.

-- 1. Rename column
ALTER TABLE public.editorial_records RENAME COLUMN author_id TO assignee_id;

-- 2. Rename foreign-key constraint
ALTER TABLE public.editorial_records
  RENAME CONSTRAINT editorial_records_author_id_fkey TO editorial_records_assignee_id_fkey;

-- 3. Rename index
ALTER INDEX IF EXISTS public.editorial_records_author_id_idx
  RENAME TO editorial_records_assignee_id_idx;

-- 4. Recreate workflows_enriched view with updated column references
...
SELECT
    ...
    -- Assignee: dedicated text column for sort + filter (RI-1146, renamed from author_email)
