-- Migration: Add ingestion version fields to workflows_enriched (RI-1242)
--
-- Exposes the relationship between:
-- - the current ingestion_record attached to the workflow, and
-- - the ingestion_record that served as the base for the editorial_record.
--
-- This lets the UI display labels like "1/4" when editorial work is based on
-- an older DI ingestion version while the workflow already points to the latest
-- ingestion record.
...
SELECT
    ...
    w.created_at,
    -- Ingestion version tracking (RI-1242)
    -- `w.ingestion_record_id` is the current/latest source for the workflow.
    -- `er.ingestion_record_id` is the source version that the editorial record
    -- was originally based on. These diverge when Data Inclusion updates create
    -- a new ingestion_record after editorial work has started.
    er.ingestion_record_id as editorial_source_ingestion_record_id,
    er_ir.version as editorial_source_version,
    ir.version as current_ingestion_version,
    CASE
        WHEN er.id IS NULL THEN false
        WHEN er.ingestion_record_id IS NULL THEN false
        WHEN w.ingestion_record_id IS NULL THEN false
        ELSE er.ingestion_record_id IS DISTINCT FROM w.ingestion_record_id
    END as is_editorial_source_outdated,
    CASE
        WHEN er.id IS NULL THEN NULL
        WHEN er_ir.version IS NULL OR ir.version IS NULL THEN NULL
        ELSE er_ir.version::text || '/' || ir.version::text
