-- Add archived_at to editorial_records.
-- Set when the fiche is archived, cleared when it is (re)published or becomes
-- compliant again. Backfilled from archive history:
--   - activity_logs (action = 'archive') — covers fiches never published
--   - publication_records (status = 'archived') — covers archives older than activity_logs

ALTER TABLE public.editorial_records
  ADD COLUMN IF NOT EXISTS archived_at timestamptz NULL;

COMMENT ON COLUMN public.editorial_records.archived_at
  IS 'Date of the last archive action. Set when online_status becomes archived, cleared on (re)publication.';

-- Backfill currently archived fiches from archive history (GREATEST ignores NULLs)
UPDATE public.editorial_records er
SET archived_at = sub.archived_at
FROM (
  SELECT
...
WHERE er.id = sub.editorial_record_id
  AND er.online_status = 'archived'
  AND sub.archived_at IS NOT NULL;

-- Recreate workflows_enriched to expose archived_at (used by the documents list)
...
SELECT
    ...
    -- Archive date (editorial_records.archived_at, set by the archive/toggle steps)
    er.archived_at,
    -- Content sources
...
COMMENT ON VIEW workflows_enriched IS
    'Enriched view of workflows combining computed statuses, presentation metadata, publication info, archive date, active/latest ingestion versions, and workflow assignment info.';
