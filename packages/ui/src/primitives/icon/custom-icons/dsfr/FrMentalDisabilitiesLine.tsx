// AUTO-GENERE — ne pas editer (sync-dsfr-icons.ts)
import type { ComponentType } from "react";

type DsfrIconProps = { color?: string; size?: number | string };

export const FrMentalDisabilitiesLine: ComponentType<DsfrIconProps> = ({
  color = "currentColor",
  size = 24,
  ...props
}) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width={size}
    height={size}
    fill={color}
    aria-hidden="true"
    {...props}
  >
    <path d="M4.8 7.3c0-.5.4-.9.9-.9s.9.4.9.9v.8h1.6v-.8C8.1 5.9 7 4.8 5.6 4.8c-1.4 0-2.4 1.1-2.4 2.5v.8h1.6v-.8Z" />
    <path d="M22.8 9.8c0-2-.8-3.6-2-4.2V1.2h-2v6.2h1c.3 0 1 .9 1 2.5 0 1.5-.6 2.4-.9 2.5h-1v1.9c-.3 4.4-2.6 6.4-7.5 6.4h-.7v-6.5h2.4c.3 0 .6-.2.8-.5.2-.3.2-.6.1-1l-3.3-7.8V1.2h-2v4c0 .1 0 .3.1.4l2.8 6.6H9.7c-.5 0-1 .4-1 1v3.1c-1-.2-1.8-.7-2.2-1.7l-.3-.7-1.5.6.3.6c.6 1.5 2 2.6 3.7 2.8v2.8H6.6c-1.3 0-2.7-1.5-3.2-2.1l-.6-.8L1.2 19l.6.8c.2.3 2.2 2.8 4.7 2.9h4.9c5.9 0 9.1-2.9 9.5-8.4V14c1.1-.6 1.9-2.2 1.9-4.2Z" />
    <path d="M14.7 7.3c0-.5.4-.9.9-.9s.9.4.9.9v.8H18v-.8c0-1.4-1.1-2.5-2.5-2.5S13.1 6 13.1 7.3v.8h1.6v-.8ZM14.9 14.5c-.5 1.1-1.5 1.8-2.7 1.8h-.8v1.6h.8c1.8 0 3.5-1.1 4.2-2.8l.3-.7-1.5-.6-.3.7Z" />
  </svg>
);
