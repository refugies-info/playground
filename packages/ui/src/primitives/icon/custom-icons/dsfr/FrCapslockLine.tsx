// AUTO-GENERE — ne pas editer (sync-dsfr-icons.ts)
import type { ComponentType } from "react";

type DsfrIconProps = { color?: string; size?: number | string };

export const FrCapslockLine: ComponentType<DsfrIconProps> = ({
  color = "currentColor",
  size = 24,
  ...props
}) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    width={size}
    height={size}
    fill={color}
    aria-hidden="true"
    {...props}
  >
    <path
      d="M20.4,2.5c0.6,0,1.1,0.5,1.1,1.1v16.9c0,0.6-0.5,1.1-1.1,1.1H3.6c-0.6,0-1.1-0.5-1.1-1.1V3.6C2.5,3,3,2.5,3.6,2.5H20.4z
	 M19.4,4.6H4.6v14.8h14.8V4.6z M15.7,14.6v2.1H8.3v-2.1H15.7z M12,6.7l3.7,3.7h-2.1v2.6h-3.2v-2.6H8.3L12,6.7z"
    />
  </svg>
);
