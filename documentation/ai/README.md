...
for (const msg of response.messages) {
  if (msg.message_type === "assistant_message") {
    ...
    console.log(msg.tool_calls.name);
    console.log(msg.tool_calls.arguments);
...
for await (const chunk of stream) {
  if (chunk.message_type === "assistant_message") {
    ...
    console.log(chunk.tool_calls.name);
    console.log(chunk.tool_calls.arguments);
  } else if (chunk.message_type === "tool_return_message") {
    console.log(chunk.toolReturn);
  } else if (chunk.message_type === "usage_statistics") {
    console.log(chunk);
  }
}
```
...
1. **Response structure:**
   - Use `message_type` NOT `type` for message type checking
   - Look for `assistant_message` message_type for agent responses
...
- **Custom Tools**: Direct Supabase access via custom Letta tools
- **Database Bridge**: Custom tools wrap Supabase Client for AI agent access
- **Language**: TypeScript
