...
erDiagram
    ...
    editorial_records ||--o{ publication_records : "published as"
    editorial_records ||--o{ content_flows : "finalizes"

    translation_records ||--o{ publication_records : "published as"
    translation_records ||--o| workflows : "context for"

    workflows ||--o| editorial_records : "manages"
    workflows ||--o| translation_records : "manages translation"
    workflows ||--o| ingestion_records : "tracks"
    ...
    publication_records {
        ...
        timestamp created_at
        timestamp updated_at
        uuid editorial_record_id FK "Link to Source (if FR)"
        uuid translation_record_id FK "Link to Translation (if translated)"
        uuid published_by FK "auth.uid()"
        text target_platform "e.g. portal"
    }
    ...
    letta_reports {
        ...
        text raw_response "Raw agent response when incomplete"
    }
    ...
    workflows {
        ...
        uuid translation_record_id FK "Optional link to translation"
    ...
    content_flows {
        ...
        text status "pending | compliant | non_compliant | error | published"
    }
...
-   **Foreign Keys**:
    -   `editorial_record_id`: References `editorial_records(id)`.
    -   `translation_record_id`: References `translation_records(id)`.
...
### `workflows`
Tracks the progress of a specific task within the pipeline. It links to ingestion, editorial, or translation records.
